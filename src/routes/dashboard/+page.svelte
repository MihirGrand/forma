<script lang="ts">
    ...
    let filteredBase = $derived(() => {
        ...
        if (currentTags.length > 0) {
            f = f.filter((file) =>
                currentTags.every((t) => file.tags.includes(t)),
            );
        }
        if (searchQuery) {
    ...
    let filteredFiles = $derived(() => {
        ...
        if (sortOrder === "newest")
            ...
            f.sort((a, b) => a.file_name.localeCompare(b.file_name));
        ...
        if (sortOrder === "newest") {
            keys.sort((a, b) => {
                const aLatest = Math.max(
                    ...groups[a].map((f) => new Date(f.created_at).getTime()),
                );
                const bLatest = Math.max(
                    ...groups[b].map((f) => new Date(f.created_at).getTime()),
                );
...
<div class="fixed inset-0 flex bg-zinc-950 overflow-hidden">
    <Sidebar
        ...
        onRemoveTag={(t: string) =>
            (currentTags = currentTags.filter((tag: string) => tag !== t))}
        onUploadClick={() => (showUploadModal = true)}
    />

    <main
        class="flex-1 flex flex-col h-full overflow-hidden bg-zinc-950 relative"
    >
        {#if $navigating}
        ...
        <!-- Top Bar -->
        <div
            class="min-h-16 border-b border-zinc-800/50 flex items-center justify-between px-8 py-3 flex-shrink-0 gap-6"
        >
            <h2
                class="text-lg font-medium text-zinc-100 capitalize font-display whitespace-nowrap"
            >
            ...
            <!-- Grid Size Toggle -->
            <div
                class="flex items-center bg-zinc-900 border border-zinc-800 rounded-lg p-1"
            >
                {#if currentType === "font"}
                    <button
                        onclick={() => (gridSize = "list")}
                        class="p-1.5 rounded-md cursor-pointer text-zinc-400 hover:text-zinc-100 {gridSize ===
                        'list'
                    ...
                    {#if groupByTag}
                        <div class="space-y-12">
                            {#each groupedFiles() || [] as group}
                                <div>
                                    <h3
                                        class="text-zinc-100 font-display text-xl font-semibold mb-4 px-2 flex items-center gap-2"
                                    >
                                        <span
                                    ...
                                    <div
                                        class="grid {gridClass} gap-6 auto-rows-max"
                                    >
                                        {#each group.files as file}
                                            ...
                                            >
                                                {#if file.type === "font"}
                                                    <div
                                                        class="w-full h-full text-zinc-100 p-2 pb-1"
                                                    >
                                                        <FontPreview
                                                            url={file.url}
                                                            name={getFontDisplayName(
                                                                file.file_name,
                                                            )}
                                                            text={previewText}
                                                            size={previewSize}
                                                ...
                                                <!-- Hover Overlay -->
                                                <div
                                                    class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none flex flex-col justify-end p-4"
                                                >
                                                    <div
                                                        class="text-sm font-medium text-white truncate drop-shadow-md mb-1"
                                                    >
                                                        {file.type === "font"
                                                            ? getFontDisplayName(
                                                                  file.file_name,
                                                    ...
                                                    >
                                                        {#each file.tags as tag}
                                                            <span
                                                                class="text-[10px] uppercase tracking-wider bg-zinc-800/80 text-zinc-300 px-1.5 py-0.5 rounded"
                                                                >{tag}</span
                                                            >
                                                        {/each}
                                                    </div>
                            ...
                            {:else}
                                <div
                                    class="h-full flex flex-col items-center justify-center text-zinc-500 border border-zinc-800 border-dashed rounded-xl py-12"
                                >
                                    <ImageIcon
                                        size={32}
                    ...
                    {:else}
                        {#if currentType === "font" && gridSize === "list"}
                            <div class="space-y-4">
                                {#each filteredFiles() as file, i (file.id)}
                                    ...
                                    <!-- svelte-ignore a11y_no_static_element_interactions -->
                                    <div
                                        class="bg-zinc-900/40 border border-zinc-800/60 rounded-xl px-4 py-4 flex items-center gap-6 cursor-pointer hover:border-amber-500/50 transition-all h-28"
                                        onclick={() => (selectedFileIndex = i)}
                                    >
                                        <div
                                            class="w-48 flex-shrink-0 flex flex-col justify-center"
                                        >
                                            <div
                                                class="text-sm font-medium text-zinc-100 truncate mb-2"
                                            >
                                                {getFontDisplayName(
                                                    file.file_name,
                                                )}
                        ...
                        {:else}
                            <div class="grid {gridClass} gap-6 auto-rows-max">
                                ...
                                {:else}
                                    <div
                                        class="col-span-full h-64 flex flex-col items-center justify-center text-zinc-500 border border-zinc-800 border-dashed rounded-xl"
                                    >
                                        <ImageIcon
                                            size={32}
                                            class="mb-4 opacity-50"
    ...
    <footer
        class="absolute bottom-0 left-[288px] right-0 border-t border-zinc-800/50 px-8 py-4 text-center text-sm text-zinc-500 bg-zinc-900"
    >
        Made with ❤️ by <a
            href="https://github.com/mihirgrand"
            target="_blank"
            rel="noopener noreferrer"
            class="text-zinc-400 hover:text-zinc-200 transition-colors">Mihir</a
        >
        | Icons by
        <a
